...
export function useHardwareClient(baseUrl = "") {
  ...
  const connectWs = useCallback(() => {
    ...
    try {
      ...
      ws.onclose = () => {
        if (!mountedRef.current) return;
        // R40: al perder el WS resetamos los indicadores derivados del backend
        // a "desconocido" para no mostrar estado stale (R39 dejaba "Conectado"
        // y los LEDs en su última posición aunque ya no tuviéramos forma de
        // saberlo). Solo wsConnected refleja el estado real medible aquí.
        // R42: además sustituimos el "Failed to fetch" técnico por un mensaje
        // amigable "Backend desconectado". Cuando el backend vuelva, el
        // primer state-broadcast con lastError:"" limpia este texto solo.
        setWsConnected(false);
        setModbusConnected(false);
        setInSafeMode(false);
        setRelays(new Array(30).fill(false));
        setLastError("Backend desconectado");
        wsRef.current = null;
 ...
 */
// R40: aceptar también ópticas peatonales y arrows que "abren paso"
// (peatón verde, flecha verde) como estados conflictivos. Antes solo
// detectaba `green` (vehicular), por eso la matriz de Hardware se quedaba
// corta y solo mostraba pares vehiculares.
const GREEN_LIKE_COLORS = new Set(["green", "ped_green", "green_arrow"]);
...
export function buildConflictPairs(geometry, mapping) {
  ...
  for (let q = 1; q <= 30; q++) {
    ...
    if (!GREEN_LIKE_COLORS.has(m.color)) continue;

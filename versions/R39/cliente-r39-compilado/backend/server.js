...
async function main() {
  ...
  async function tryConnect() {
    try {
      ...
      startPolling();
      broadcast({ type: "state", relays: lastRelayState, connected: true, lastError: "" });
      ...
      broadcast({ type: "state", relays: lastRelayState, connected: false, lastError });
      scheduleReconnect();
    }
  }
  ...
  function startPolling() {
    ...
    pollingTimer = setInterval(async () => {
      ...
      try {
        ...
        if (changed) {
          broadcast({ type: "state", relays: state, connected: true, lastError: "" });
        }
      } catch (e) {
        lastError = e.message;
      }
  ...
  const safety = new SafetyMonitor({
    driver,
    timeoutMs: config.modbus.heartbeatTimeoutMs || 3000,
    onSafeMode: (info) => {
      ...
      if (incoming.modbus) {
        ...
        if (incoming.modbus.simulationMode !== undefined && incoming.modbus.simulationMode !== wasSimMode) {
          ...
          if (config.modbus.simulationMode) {
            ...
            if (simulator) { await simulator.stop(); simulator = null; }
  ...
  app.post("/api/all-off", async (req, res) => {
    try {
      ...
      broadcast({ type: "state", relays: lastRelayState, connected: driver.connected, lastError });

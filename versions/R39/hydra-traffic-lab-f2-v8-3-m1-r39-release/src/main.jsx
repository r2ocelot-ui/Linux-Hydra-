...
const HYDRA_VERSION_LABEL = "F2-V8.3-M1-R39";
const HYDRA_VERSION_SLUG = "f2-v8-3-m1-r39-release";
...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      // R39: emite tick al backend Modbus para el cruce activo si el control
      // hardware está habilitado y hay un mapping definido. El backend traduce
      // headStates → relés físicos vía Modbus TCP (o simulador).
      if (hardwareConfig.modbusEnabled && selected) {
        try {
          const sig = getSignal(selected, tick + 1, settings);
          const mapping = (hardwareConfig.mappings || {})[selected.id];
          if (mapping) {
            const targetRelays = buildTargetRelays(mapping, sig.headStates);
            hardwareClient.sendTick(selected.id, targetRelays);
          }
  ...
  }, [running, tick, settings, hardwareConfig, selected, hardwareClient]);

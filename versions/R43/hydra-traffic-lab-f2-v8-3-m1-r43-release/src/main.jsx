...
import { useHardwareClient, buildTargetRelays, buildConflictPairs } from "./hardware-client.js";
...
const HYDRA_VERSION_LAST_KEY = "hydraTrafficLab.f2.lastVersion";

// R30: tiempos de inactividad en milisegundos. Configurables.
const HYDRA_INACTIVITY_TOTAL_MS = 15 * 60 * 1000; // 15 min total para cerrar sesion
const HYDRA_INACTIVITY_WARN_AT = [
  { remainingMs: 10 * 60 * 1000, kind: "soft", label: "Te faltan 10 minutos" },
  { remainingMs: 5 * 60 * 1000, kind: "soft", label: "Te faltan 5 minutos" },
  { remainingMs: 1 * 60 * 1000, kind: "countdown", label: "Sesion se cerrara en" },
];
const HYDRA_MAX_RESTORE_POINTS = 10;
const HYDRA_SECTIONS = ["Inicio", "Mapa del sistema", "Cruces", "Coordinación", "Escenarios", "Informes", "Eventos", "Dispositivos", "Hardware", "Cámaras", "Configuración", "Usuarios", "Sistema"];
const HYDRA_VERSION_LABEL = "F2-V8.3-M1-R43";
const HYDRA_VERSION_SLUG = "f2-v8-3-m1-r43-release";
// R30: hash y fecha inyectados por vite.config.js en build time.
// Fallback "dev" para arranques de dev server fuera de git.
const HYDRA_BUILD_HASH = typeof __BUILD_HASH__ !== "undefined" ? __BUILD_HASH__ : "dev";
const HYDRA_BUILD_DATE = typeof __BUILD_DATE__ !== "undefined" ? __BUILD_DATE__ : new Date().toISOString();
...
const HYDRA_PERMISSION_RULES = {
  ...
  users: { permission: "manageUsers", label: "Administrar usuarios globales, proyectos y guardado critico" },
  projectUsers: { permission: "manageProjectUsers", label: "Administrar usuarios asignados a la ciudad activa" },
...
// R28: simulacion del modulo Advantech/PLC para Mapa de E/S DI/DO.
// El modelo es derivado: los DI/DO se calculan en vivo del estado del cruce
// y la senal actual; no hay edicion del mapeo todavia (R29 si toca).
const HYDRA_IO_MODULE = { name: "Advantech ADAM-6052 (simulado)", diSlots: 16, doSlots: 16 };
const HYDRA_DI_KIND_LABELS = {
  lazo: "Lazo",
  pulsador: "Pulsador peatonal",
  selector: "Selector manual/auto",
  alarma: "Alarma",
};
const HYDRA_DO_OPTIC_LABELS = {
  red: "Rojo",
  amber: "Ambar",
  amber_1: "Ambar 1",
  amber_2: "Ambar 2",
  green: "Verde",
  red_arrow: "Flecha roja",
  amber_arrow: "Flecha ambar",
  green_arrow: "Flecha verde",
  ped_red: "Peaton rojo",
  ped_green: "Peaton verde",
};
const HYDRA_IO_HELP = [
  "Mapa de E/S simula un modulo Advantech/PLC con 16 entradas digitales y 16 salidas digitales.",
  "Las entradas DI se derivan en vivo de llegadas, peticiones peatonales, selector manual y alarmas hardware.",
  "Las salidas DO reflejan el estado actual de cada optica del cruce: rojo, ambar, verde, flechas y peaton.",
  "En V9 el mismo mapa pasara a leer/escribir E/S real de laboratorio sobre el regulador local.",
];
...
function projectIdsForDefaults(projects = []) {
  const firstProjectId = projects[0]?.id || "project-local";
  return [firstProjectId];
}

function makeDefaultHydraUsers(projects = []) {
  // R32: cuenta unica por defecto. El nombre del usuario es "Hydra System"
  // y el rol asociado es el de Administrador del sistema (superadmin).
  return [
    {
      id: "user-hydra-system",
      name: "Hydra System",
      username: "Hydra System",
      roleId: "superadmin",
      pin: "9999",
      password: "",
      projectIds: ["*"],
...
function normalizeHydraUser(user, index = 0, projects = []) {
  const defaults = makeDefaultHydraUsers(projects)[index] || makeDefaultHydraUsers(projects)[0];
  const roleId = getRoleById(user?.roleId) ? user.roleId : defaults.roleId;
  const projectIds = Array.isArray(user?.projectIds) && user.projectIds.length ? user.projectIds : defaults.projectIds;
  return {
    id: String(user?.id || defaults.id || `user-${index + 1}`),
    name: String(user?.name || defaults.name || `Usuario ${index + 1}`),
    username: String(user?.username || defaults.username || `usuario${index + 1}`).trim() || `usuario${index + 1}`,
    roleId,
    pin: String(user?.pin ?? defaults.pin ?? "0000"),
    // R35: contraseña separada del PIN (libre: letras, números, símbolos).
    // Vacía = solo se exige PIN (compatibilidad con cuentas previas).
    password: String(user?.password ?? defaults.password ?? ""),
    projectIds,
    active: user?.active !== false,
    projectPermissions: user?.projectPermissions && typeof user.projectPermissions === "object" ? user.projectPermissions : {},
    // R30: stub MFA y marca de cuenta por defecto.
    requiresMFA: user?.requiresMFA === true,
    isDefaultAccount: user?.isDefaultAccount === true || defaults.isDefaultAccount === true,
  };
}

function loadHydraUsers(projects = []) {
  const defaults = makeDefaultHydraUsers(projects);
  if (typeof window === "undefined" || !storageAvailable()) return defaults;
  try {
    const raw = window.localStorage.getItem(HYDRA_USERS_KEY);
    if (!raw) return defaults;
    const data = JSON.parse(raw);
    if (!Array.isArray(data)) return defaults;
    const normalized = data.map((user, index) => normalizeHydraUser(user, index, projects));
    const existingIds = new Set(normalized.map((user) => user.id));
    const missingDefaults = defaults.filter((user) => !existingIds.has(user.id));
    return [...normalized, ...missingDefaults];
  } catch (error) {
    console.warn("No se pudieron cargar usuarios Hydra:", error);
    return defaults;
  }
}

function saveHydraUsers(users) {
  if (typeof window === "undefined" || !storageAvailable()) return false;
  window.localStorage.setItem(HYDRA_USERS_KEY, JSON.stringify(users));
  return true;
}
...
function getFirstUserForRole(users, roleId) {
  return users.find((user) => user.roleId === roleId && user.active !== false) || null;
}
...
function visibleProjectsForUser(projects, user) {
  if (!user) return [];
  if (user.projectIds?.includes("*")) return projects;
  return projects.filter((project) => userCanAccessProject(user, project.id));
}

function effectivePermissionsForUser(user, projectId, baseRole) {
  const base = new Set(baseRole?.permissions || []);
  const projectPermissions = user?.projectPermissions?.[projectId] || [];
  const globalProjectPermissions = user?.projectPermissions?.["*"] || [];
  [...projectPermissions, ...globalProjectPermissions].forEach((permission) => {
    if (HYDRA_PERMISSION_LABELS[permission]) base.add(permission);
  });
  return [...base];
}
...
function canManageTargetUser(manager, managerRole, target, projectId) {
  if (!manager || !managerRole || !target) return false;
  if (roleHasPermission(managerRole, "manageUsers")) return true;
  if (!roleHasPermission(managerRole, "manageProjectUsers")) return false;
  if (!projectId || !userCanAccessProject(manager, projectId) || !userCanAccessProject(target, projectId)) return false;
  return roleLevel(manager.roleId) > roleLevel(target.roleId);
}

function makeAuditEntryFor(user, role, project, category, action, target = "", result = "ok", permission = "", detail = "") {
  const now = new Date();
  return {
    ...
    roleId: role?.id || user?.roleId || "sin-rol",
    roleName: role?.name || getRoleById(user?.roleId)?.name || "Sin rol",
    projectId: project?.id || "sin-proyecto",
...
// R36: normaliza un cruce cargado de localStorage/import. Recupera estado
// corrupto (p. ej. geometry guardada como string tras el bug del selector
// en R35) y garantiza que existen los campos que usa el motor de señal y
// el render, sin perder los valores guardados por el usuario.
function normalizeCrossing(raw, index = 0) {
  const source = raw && typeof raw === "object" ? raw : {};
  const id = String(source.id || `X${index + 1}`);
  const lat = Number.isFinite(Number(source.lat)) ? Number(source.lat) : CENTER[0];
  const lng = Number.isFinite(Number(source.lng)) ? Number(source.lng) : CENTER[1];
  const base = makeCrossing(id, source.name || `Cruce ${id}`, lat, lng, index, geometryValue(source.geometry));
  const geometryOk = source.geometry && typeof source.geometry === "object"
    && Array.isArray(source.geometry.phases) && Array.isArray(source.geometry.signalGroups);
  return {
    ...base,
    ...source,
    id,
    lat,
    lng,
    geometry: geometryOk ? source.geometry : makeGeometry(geometryValue(source.geometry)),
    arrivals: source.arrivals && typeof source.arrivals === "object" ? { ...base.arrivals, ...source.arrivals } : base.arrivals,
    queues: source.queues && typeof source.queues === "object" ? { ...base.queues, ...source.queues } : base.queues,
    pedestrianRequests: source.pedestrianRequests && typeof source.pedestrianRequests === "object" ? { ...base.pedestrianRequests, ...source.pedestrianRequests } : base.pedestrianRequests,
    hardware: source.hardware && typeof source.hardware === "object" ? source.hardware : base.hardware,
    cameras: Array.isArray(source.cameras) ? source.cameras : [],
  };
}

function normalizeProjectSnapshot(snapshot = {}) {
  const rawCrossings = Array.isArray(snapshot.crossings) ? snapshot.crossings : [];
  const crossings = rawCrossings.map((crossing, index) => normalizeCrossing(crossing, index));
  return {
    crossings,
    ...
    hardware: normalizeHardwareConfig(snapshot.hardware),
  };
}
...
function normalizeHardwareConfig(raw) {
  ...
  return {
    modbusEnabled: !!src.modbusEnabled,
    backendBaseUrl: typeof src.backendBaseUrl === "string" ? src.backendBaseUrl : "",
    modbusHost: typeof src.modbusHost === "string" ? src.modbusHost : "192.168.1.44",
    modbusPort: Number.isInteger(src.modbusPort) ? src.modbusPort : 502,
    modbusUnit: Number.isInteger(src.modbusUnit) ? src.modbusUnit : 1,
    simulationMode: src.simulationMode === undefined ? true : !!src.simulationMode,
    flashMs: Number.isInteger(src.flashMs) ? src.flashMs : 5000,
    mappings: src.mappings && typeof src.mappings === "object" ? src.mappings : {},
  };
}
...
function loadHydraSession() {
  ...
  try {
    ...
    if (!session?.userId && (!session?.roleId || !getRoleById(session.roleId))) return null;
    return session;
  } catch {
    return null;
  }
...
// Punto de extensión multi-factor para V9. Firma final ya pensada para
// PIN + contraseña + patrón + TOTP. En R30 solo el PIN se valida; los
// demás factores son stub: el backend real (V9) los enchufará con bcrypt/
// argon2 + emisor TOTP. La función mantiene firma estable para que el
// reemplazo sea drop-in.
//
// Reglas R30:
// - PIN siempre se valida en cliente (compatibilidad con cuentas demo).
// - Si user.requiresMFA = true, los demás factores son obligatorios pero
//   se aceptan en stub mientras no haya backend (auditado como tal).
function verifyCredentials(user, pin, password = "", pattern = "", totpCode = "") {
  if (!user || user.active === false) return { ok: false, reason: "inactive" };
  const pinOk = String(pin ?? "").trim() === String(user.pin ?? "");
  if (!pinOk) return { ok: false, reason: "pin" };
  // R35: si el usuario tiene contraseña configurada (no vacía), debe
  // coincidir además del PIN. Si no la tiene, solo se exige PIN.
  const userPassword = String(user.password ?? "");
  if (userPassword.length > 0 && String(password ?? "") !== userPassword) {
    return { ok: false, reason: "password" };
  }
  if (user.requiresMFA) {
    // Stub V9: en cliente aceptamos cualquier valor no vacío hasta que
    // exista backend. Marcamos el resultado para auditoría.
    const passwordProvided = String(password ?? "").length > 0;
    const patternProvided = String(pattern ?? "").length > 0;
    const totpProvided = String(totpCode ?? "").length > 0;
    if (!passwordProvided && !patternProvided && !totpProvided) {
      return { ok: false, reason: "mfa-missing" };
    }
    return { ok: true, mfa: "stub", factors: { passwordProvided, patternProvided, totpProvided } };
  }
  return { ok: true, mfa: "none" };
}
...
const AMBER = 4;
const ALL_RED = 3;
// R42: timings peatonales realistas. Dentro del verde vehicular, los
// peatones tienen los ultimos PED_FLASH_SEC segundos parpadeando como
// aviso de "termina el cruce", y luego PED_CLEARANCE_SEC en rojo solido
// antes de que cambie la fase. Asi nunca quedan peatones a medio cruzar
// cuando se va a abrir el verde a los coches perpendiculares.
const PED_FLASH_SEC = 5;
const PED_CLEARANCE_SEC = 3;
...
// R32: hook de inactividad sin causar re-renders en cada mousemove.
// El bug en R30/R31 era que setLastActivity(Date.now()) se llamaba en cada
// evento de raton, re-renderizando la App entera. Eso interrumpia el drag
// de los marcadores Leaflet, devolviendolos a su posicion original a mitad
// del arrastre. Ahora lastActivity vive en useRef (no provoca render) y
// solo se actualiza el estado cuando cambia realmente el aviso visible.
function useInactivityTimeout({ active, totalMs, warns, onExpire }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const triggered = warns.find((w) => remaining <= w.remainingMs);
      setWarning((prev) => {
        if (!triggered) return prev === null ? prev : null;
        if (!prev || prev.kind !== triggered.kind ||
            Math.floor((prev.remainingMs || 0) / 1000) !== Math.floor(remaining / 1000)) {
          return { ...triggered, remainingMs: remaining };
        }
        return prev;
      });
    }, 1000);
    return () => clearInterval(interval);
  }, [active, totalMs, warns]);
...
// R30: dialogo que aparece la primera vez que se carga una nueva revision.
// Solo durante desarrollo R-versions; cuando el producto pase a final
// estable se quita y la persistencia es automatica por usuario.
function RevisionResetDialog({ open, previousVersion, currentVersion, onReset, onKeep }) {
  if (!open) return null;
  return (
    <div className="access-modal-backdrop" role="presentation">
      <div className="access-modal" role="dialog" aria-modal="true" aria-label="Nueva revision detectada">
        <div className="access-modal-header">
          <div>
            <span>Nueva revision</span>
            <h2>Hemos detectado una version nueva</h2>
          </div>
        </div>
        <div className="revision-reset-body">
          <p>
            La ultima version cargada en este navegador era
            {" "}<code>{previousVersion || "ninguna"}</code>{" "}
            y ahora estamos en
            {" "}<code>{currentVersion}</code>.
          </p>
          <p>
            Mientras estamos en revisiones (R), puedes empezar limpio para probar
            como si fueras un usuario nuevo, o conservar tus datos actuales
            (proyectos, usuarios, auditoria).
          </p>
          <p className="access-warning">
            Cuando el producto entre en version estable este aviso desaparecera y
            la persistencia se mantendra siempre.
          </p>
        </div>
        <div className="access-actions">
          <Button type="button" onClick={onReset}>Reset (recomendado para probar)</Button>
          <Button type="button" variant="secondary" onClick={onKeep}>Conservar datos</Button>
        </div>
      </div>
    </div>
  );
...
function InactivityWarningModal({ warning, onDismiss }) {
  if (!warning) return null;
  const minutes = Math.floor(warning.remainingMs / 60000);
  const seconds = Math.floor((warning.remainingMs % 60000) / 1000);
  const isCountdown = warning.kind === "countdown";
  const display = isCountdown
    ? `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`
    : `${minutes} min`;
  return (
    <div className="access-modal-backdrop inactivity-warning-backdrop" role="presentation">
      <div className="access-modal inactivity-warning" role="alertdialog" aria-modal="true">
        <div className="access-modal-header">
          <div>
            <span>Inactividad</span>
            <h2>{warning.label}{isCountdown ? "" : ""}</h2>
          </div>
          <button type="button" onClick={onDismiss} aria-label="Continuar sesion">x</button>
        </div>
        <div className="inactivity-warning-body">
          <div className={`inactivity-warning-clock ${isCountdown ? "countdown" : ""}`}>{display}</div>
          <p>
            {isCountdown
              ? "La sesion se cerrara automaticamente cuando llegue a cero. Mueve el raton, pulsa una tecla o cierra este aviso para continuar."
              : "Mueve el raton, pulsa una tecla o cierra este aviso para reiniciar el contador."}
          </p>
        </div>
        <div className="access-actions">
          <Button type="button" onClick={onDismiss}>Continuar sesion</Button>
        </div>
      </div>
    </div>
  );
...
// R42: añadido "double_amber" como estado alternante (amber_1 ↔ amber_2
// a 1Hz) que se usa para señales de incorporación tipo "ceda paso" en
// vías secundarias. Antes solo existian amber_1 y amber_2 como estados
// fijos (uno o el otro); ahora "double_amber" significa "ambos alternados".
// Las etiquetas amber_1/amber_2 quedan como numericas explicitas, no
// como "doble ambar" que era ambiguo.
const OUTPUT_STATES = {
  red: "Rojo",
  green: "Verde",
  amber: "Ámbar",
  amber_1: "Ámbar 1",
  amber_2: "Ámbar 2",
  double_amber: "Doble ámbar (intermitente)",
...
function outputStateForHead(head, requested) {
  ...
  // R42: "double_amber" como estado alternante. Aqui devolvemos un
  // fallback estable (amber_1 si lo tiene). La alternancia real 1Hz se
  // computa en getSignal() porque alli tenemos acceso al tick.
  if (requested === "double_amber") {
    if (head.optics.includes("amber_1") && head.optics.includes("amber_2")) return "amber_1";
    if (head.optics.includes("amber")) return "amber";
    if (head.optics.includes("amber_1")) return "amber_1";
    if (head.optics.includes("amber_2")) return "amber_2";
  }
...
function getSignal(crossing, tick, settings) {
  ...
  // R42: dentro del stage "green" calculamos sub-stages para peatones
  // (solid → flashing → clearance). El parpadeo aprovecha la paridad de
  // tick (1Hz). Para "double_amber" la alternancia es amber_1↔amber_2 al
  // mismo ritmo. Ambos comportamientos se aplican aqui antes de resolver
  // el color final con outputStateForHead/headStateFromGroupState.
  // R43: la alternancia "double_amber" sale del if(stage==="green") y se
  // aplica en cualquier stage (green, amber, allRed). En R42 el aviso
  // parpadeante se congelaba en amber_1 durante los ~7s de transicion
  // entre fases (AMBER 4s + ALL_RED 3s). Como el doble ambar es un aviso
  // PERMANENTE (ceda paso, salida, obras...), debe seguir parpadeando
  // ininterrumpidamente durante todo el ciclo.
  const tickEven = Math.floor(tick) % 2 === 0;
  let pedSubStage = "solid"; // solid | flashing | clearance
  if (stage === "green" && stageLength > 0) {
    const flashStartAt = Math.max(0, stageLength - PED_FLASH_SEC - PED_CLEARANCE_SEC);
    const clearanceStartAt = Math.max(0, stageLength - PED_CLEARANCE_SEC);
    if (elapsed >= clearanceStartAt) pedSubStage = "clearance";
    else if (elapsed >= flashStartAt) pedSubStage = "flashing";
  }
  ...
  normalizedGeometry.signalHeads.forEach((head) => {
    ...
    // R43: caso prioritario sobre el resto de stages. Cualquier head con
    // optica amber_1+amber_2 al que se pida "double_amber" parpadea a 1Hz
    // sin importar si la fase activa esta en green/amber/allRed.
    if (requestedState === "double_amber" && head.optics.includes("amber_1") && head.optics.includes("amber_2")) {
      resolved = tickEven ? "amber_1" : "amber_2";
    }
    else if (stage === "green") {
      // Caso peatón con verde pedido durante el stage green.
      // Aplica sub-stage: solid → flashing (1Hz) → clearance (rojo).
      if (head.groupType === "pedestrian" && (requestedState === "ped_green" || requestedState === "green") && head.optics.includes("ped_green")) {
        if (pedSubStage === "clearance") {
          resolved = head.optics.includes("ped_red") ? "ped_red" : outputStateForHead(head, "ped_red");
        } else if (pedSubStage === "flashing") {
          resolved = tickEven ? "ped_green" : (head.optics.includes("ped_red") ? "ped_red" : "off");
        } else {
          resolved = outputStateForHead(head, requestedState);
        }
      }
      // Cualquier otro head durante green → comportamiento normal.
      else {
        resolved = outputStateForHead(head, requestedState);
      }
    } else {
      resolved = headStateFromGroupState(head, groupState);
    }

    headStates[head.id] = resolved;
  });
  ...
  return {
    ...
    pedSubStage, // R42: solid | flashing | clearance (solo informativo, util para UI)
  };
}
...
function Button({ children, onClick, variant = "primary", disabled = false, type = "button", title = "", className = "" }) {
  // R34: si el children es texto y no llega title explicito, usamos el
  // texto como tooltip nativo. Asi cuando una celda estrecha trunca con
  // ellipsis, el usuario ve la etiqueta completa al pasar el raton.
  const autoTitle = title || (typeof children === "string" ? children : "");
  return <button type={type} onClick={onClick} disabled={disabled} title={autoTitle} className={`btn btn-${variant} ${className}`.trim()}>{children}</button>;
}
    ...
    <Panel className="map-panel">
      <div className="panel-header">
        <div className="section-page-title">
          ...
          <HelpButton title="Mapa operativo" items={[
            "Minimapa con iconos de semaforo/peaton y lampara de color por cabeza activa.",
            "Modo anadir cruce: pulsa en el mapa para soltar un cruce nuevo.",
            "Selecciona dos cruces y crea/quita enlace manual.",
            "El icono cambia segun el grupo (vehiculo/peaton/giro) y el modo (manual/local/averia).",
          ]} />
        </div>
      </div>
    ...
    <Panel className="crossing-control-wide">
      ...
      <div className="crossing-control-wide-grid">
        ...
        <div className="identity-clean-grid">
          ...
          <label className="compact-field">
            <span>Geometría</span>
            <select value={geometryValue(selected.geometry)} onChange={(event) => onSetGeometry(selected.id, event.target.value)}>
...
function HardwarePanel({ selected, activeProject, hardwareClient, onUpdateHardwareConfig, onUpdateMapping, activeRole, onPermissionDenied }) {
  const hwConfig = activeProject?.hardware || {};
  const mapping = (hwConfig.mappings || {})[selected?.id] || {};
  const heads = selected?.geometry?.signalHeads || [];

  const [draftHost, setDraftHost] = useState(hwConfig.modbusHost || "192.168.1.44");
  const [draftPort, setDraftPort] = useState(String(hwConfig.modbusPort || 502));
  const [draftUnit, setDraftUnit] = useState(String(hwConfig.modbusUnit || 1));
  const [draftFlashMs, setDraftFlashMs] = useState(String(hwConfig.flashMs || 5000));
  const [draftBackend, setDraftBackend] = useState(hwConfig.backendBaseUrl || "");
  const [busy, setBusy] = useState(false);
  const [lastTestAt, setLastTestAt] = useState(null);

  useEffect(() => {
    setDraftHost(hwConfig.modbusHost || "192.168.1.44");
    setDraftPort(String(hwConfig.modbusPort || 502));
    setDraftUnit(String(hwConfig.modbusUnit || 1));
    setDraftFlashMs(String(hwConfig.flashMs || 5000));
    setDraftBackend(hwConfig.backendBaseUrl || "");
  }, [hwConfig.modbusHost, hwConfig.modbusPort, hwConfig.modbusUnit, hwConfig.flashMs, hwConfig.backendBaseUrl]);

  const allowed = roleHasPermission(activeRole, "maintainHardware");
  const colorOptions = ["red", "amber", "green", "ped_red", "ped_green", "red_arrow", "amber_arrow", "green_arrow", "amber_1", "amber_2"];
  const colorLabels = { red: "Rojo", amber: "Ámbar", green: "Verde", ped_red: "Peatón rojo", ped_green: "Peatón verde", red_arrow: "Flecha roja", amber_arrow: "Flecha ámbar", green_arrow: "Flecha verde", amber_1: "Ámbar 1", amber_2: "Ámbar 2" };

  async function saveConfig() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "guardar configuración hardware"); return; }
    setBusy(true);
    try {
      const updated = {
        ...hwConfig,
        modbusHost: draftHost.trim() || "192.168.1.44",
        modbusPort: parseInt(draftPort, 10) || 502,
        modbusUnit: parseInt(draftUnit, 10) || 1,
        flashMs: Math.max(500, Math.min(60000, parseInt(draftFlashMs, 10) || 5000)),
        backendBaseUrl: draftBackend.trim(),
      };
      onUpdateHardwareConfig?.(updated);
      // Empujamos al backend
      await hardwareClient.updateConfig({
        modbus: {
          host: updated.modbusHost,
          port: updated.modbusPort,
          unitId: updated.modbusUnit,
          flashMs: updated.flashMs,
          simulationMode: hwConfig.simulationMode === undefined ? true : !!hwConfig.simulationMode,
        },
      });
    } catch (e) {
      // El client hook ya pone lastError
    } finally {
      setBusy(false);
    }
  }
  ...
  async function toggleSimulation() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "cambiar modo simulación"); return; }
    setBusy(true);
    try {
      const newMode = !hwConfig.simulationMode;
      onUpdateHardwareConfig?.({ ...hwConfig, simulationMode: newMode });
      await hardwareClient.updateConfig({
        modbus: {
          simulationMode: newMode,
          host: hwConfig.modbusHost || "192.168.1.44",
          port: hwConfig.modbusPort || 502,
          unitId: hwConfig.modbusUnit || 1,
        },
      });
  ...
  async function doConnect() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "conectar al módulo"); return; }
    setBusy(true);
    try { await hardwareClient.connect(); } finally { setBusy(false); }
  }
  async function doDisconnect() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "desconectar módulo"); return; }
    setBusy(true);
    try { await hardwareClient.disconnect(); } finally { setBusy(false); }
  }
  async function doAllOff() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "apagar todos los relés"); return; }
    setBusy(true);
    try { await hardwareClient.allOff(); } finally { setBusy(false); }
  }
  async function doTestRelay(n) {
    if (!allowed) { onPermissionDenied?.("maintainHardware", `probar relé Q${n}`); return; }
    setLastTestAt({ n, ts: Date.now() });
    try { await hardwareClient.testRelay(n, 500); } catch (e) {}
  }

  function updateMappingCell(qKey, field, value) {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "editar mapeo Q→head"); return; }
    const current = mapping[qKey] || {};
    const next = { ...current };
    if (value === "") delete next[field]; else next[field] = value;
    if (!next.headId && !next.color) {
      const newMap = { ...mapping };
      delete newMap[qKey];
      onUpdateMapping?.(selected.id, newMap);
    } else {
      onUpdateMapping?.(selected.id, { ...mapping, [qKey]: next });
    }
  }

  function autofillMapping() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "auto-rellenar mapeo"); return; }
    const next = {};
    let q = 1;
    for (const head of heads) {
      const opts = head.optics || [];
      // Orden estándar: rojo, ámbar, verde (lo que tenga el head)
      const order = ["red", "amber", "green", "ped_red", "ped_green", "red_arrow", "amber_arrow", "green_arrow"];
      const colorsInHead = order.filter((c) => opts.includes(c));
      for (const color of colorsInHead) {
        if (q > 30) break;
        next[`Q${q}`] = { headId: head.id, color };
        q++;
      }
      if (q > 30) break;
    }
    onUpdateMapping?.(selected.id, next);
  }

  function clearMapping() {
    if (!allowed) { onPermissionDenied?.("maintainHardware", "limpiar mapeo"); return; }
    onUpdateMapping?.(selected.id, {});
  }

  const conflictPairs = useMemo(() => buildConflictPairs(selected?.geometry, mapping), [selected?.geometry, mapping]);

  // R41 fix: limpieza automática de mapeo Q→head huérfano cuando se
  // borran o renombran heads desde el editor de Geometría. En R40, si el
  // usuario eliminaba un grupo (lo que arrastraba sus maniobras) o
  // cambiaba el tipo, las Qs mapeadas a esos heads quedaban apuntando a
  // IDs inexistentes: el envío de fases las ignoraba silenciosamente, lo
  // que parecía que "el sistema no funcionaba". Ahora detectamos
  // entradas huérfanas y las reseteamos a "sin asignar".
  useEffect(() => {
    if (!selected?.id || !mapping) return;
    const validHeadIds = new Set(heads.map((h) => h.id));
    const cleaned = {};
    let changed = false;
    for (const [qKey, m] of Object.entries(mapping)) {
      if (m && m.headId && validHeadIds.has(m.headId)) {
        cleaned[qKey] = m;
      } else if (m && (m.headId || m.color)) {
        // entrada con datos pero head desaparecido → la quitamos
        changed = true;
      }
    }
    if (changed) {
      onUpdateMapping?.(selected.id, cleaned);
    }
  }, [heads, selected?.id, mapping, onUpdateMapping]);

  // Envía la matriz de conflictos al backend cuando cambia
  useEffect(() => {
    if (!hardwareClient.wsConnected) return;
    hardwareClient.sendSetConfig({ conflictPairs });
  }, [hardwareClient.wsConnected, conflictPairs, hardwareClient.sendSetConfig]);
  ...
  return (
    <div className="hardware-panel">
      <Panel>
        <div className="panel-header">
          <div>
            <h2>Hardware — Módulo Modbus TCP</h2>
            <p>Control de relés físicos del cruce activo a través del backend local. Los relés mapean a las salidas Q1-Q30 del módulo.</p>
          </div>
          <div className="panel-tools">
            <HelpButton title="Hardware" items={[
              "El frontend envía cada segundo el estado deseado de Q1-Q30 al backend Node.js local (ws://localhost:3001/ws).",
              "El backend traduce a Modbus TCP y manda los comandos al módulo Waveshare 30CH en 192.168.1.44:502.",
              "Modo simulación: el backend usa un módulo falso interno, sin red. Ideal para desarrollo.",
              "Comando Flash ON: cada relé tiene su propio watchdog interno; si el backend muere, los relés se apagan solos.",
              "Conflict matrix: si dos Qs incompatibles intentan encenderse a la vez → all-off automático + alarma.",
              "Requiere permiso maintainHardware para todas las acciones.",
            ]} />
          </div>
        </div>

        <div className="hw-status-grid">
          <div className={`hw-pill ${hardwareClient.wsConnected ? "ok" : "bad"}`}>
            <span>Backend</span>
            <strong>{hardwareClient.wsConnected ? "Conectado" : "Desconectado"}</strong>
          </div>
          <div className={`hw-pill ${hardwareClient.modbusConnected ? "ok" : "bad"}`}>
            <span>Módulo Modbus</span>
            <strong>{hardwareClient.modbusConnected ? "Conectado" : "Desconectado"}</strong>
          </div>
          <div className="hw-pill">
            <span>Modo</span>
            <strong>{hwConfig.simulationMode ? "SIMULACIÓN" : "REAL"}</strong>
          </div>
          <div className={`hw-pill ${hwConfig.modbusEnabled ? "ok" : ""}`}>
            <span>Control hardware</span>
            <strong>{hwConfig.modbusEnabled ? "Activado" : "Desactivado"}</strong>
          </div>
          {hardwareClient.inSafeMode && (
            <div className="hw-pill bad"><span>Estado</span><strong>SAFE MODE</strong></div>
          )}
          {hardwareClient.lastError && (
            <div className="hw-pill bad" title={hardwareClient.lastError}><span>Último error</span><strong>{hardwareClient.lastError.slice(0, 40)}</strong></div>
          )}
        </div>

        <div className="hw-actions-row">
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={doConnect} disabled={busy} action="conectar módulo">Conectar</PermissionButton>
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={doDisconnect} disabled={busy} action="desconectar módulo" variant="secondary">Desconectar</PermissionButton>
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={doAllOff} disabled={busy} action="apagar todos los relés" variant="danger">Apagar todos los relés</PermissionButton>
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={toggleSimulation} disabled={busy} action="cambiar modo simulación" variant="secondary">{hwConfig.simulationMode ? "Pasar a REAL" : "Pasar a SIMULACIÓN"}</PermissionButton>
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={toggleEnabled} disabled={busy} action="activar control hardware">{hwConfig.modbusEnabled ? "Detener envío de fases" : "Empezar envío de fases"}</PermissionButton>
        </div>
      </Panel>

      <Panel>
        <div className="panel-header"><div><h3>Configuración del módulo</h3><p>Datos del módulo Waveshare 30CH. Cambios se aplican al backend y se persisten en el proyecto.</p></div></div>
        <div className="hw-config-grid">
          <label className="field"><span>IP del módulo</span><input type="text" value={draftHost} onChange={(e) => setDraftHost(e.target.value)} placeholder="192.168.1.44" /></label>
          <label className="field"><span>Puerto</span><input type="number" value={draftPort} onChange={(e) => setDraftPort(e.target.value)} placeholder="502" /></label>
          <label className="field"><span>Unit ID</span><input type="number" min="1" max="255" value={draftUnit} onChange={(e) => setDraftUnit(e.target.value)} placeholder="1" /></label>
          <label className="field"><span>Flash duration (ms)</span><input type="number" min="500" max="60000" step="500" value={draftFlashMs} onChange={(e) => setDraftFlashMs(e.target.value)} /></label>
          <label className="field" style={{ gridColumn: "1 / -1" }}><span>Backend URL (vacío = mismo origen)</span><input type="text" value={draftBackend} onChange={(e) => setDraftBackend(e.target.value)} placeholder="http://localhost:3001" /></label>
        </div>
        <div className="hw-actions-row">
          <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={saveConfig} disabled={busy} action="guardar configuración modbus">Guardar configuración</PermissionButton>
        </div>
      </Panel>

      <Panel>
        <div className="panel-header"><div><h3>Pruebas manuales · Q1-Q30</h3><p>Cada relé acepta un flash de 500 ms para verificar el cableado. El indicador refleja el estado real leído del módulo cada 300 ms.</p></div></div>
        <div className="hw-relay-grid">
          {Array.from({ length: 30 }, (_, i) => {
            const n = i + 1;
            const on = !!hardwareClient.relays[i];
            const justTested = lastTestAt && lastTestAt.n === n && (Date.now() - lastTestAt.ts) < 1500;
            return (
              <div key={n} className={`hw-relay-cell ${on ? "on" : ""} ${justTested ? "test" : ""}`}>
                <div className="hw-relay-label">
                  <span className={`hw-led ${on ? "on" : ""}`} />
                  <strong>Q{n}</strong>
                </div>
                <Button variant="secondary" onClick={() => doTestRelay(n)}>Probar</Button>
              </div>
            );
          })}
        </div>
      ...
      <Panel>
        <div className="panel-header">
          <div><h3>Mapeo Q1-Q30 → óptica del cruce «{selected?.name || "—"}»</h3><p>Define a qué semáforo (head) y a qué color responde cada salida Q.</p></div>
          <div className="panel-tools">
            <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={autofillMapping} action="auto-rellenar mapeo" variant="secondary">Auto-rellenar</PermissionButton>
            <PermissionButton role={activeRole} permission="maintainHardware" onDenied={onPermissionDenied} onClick={clearMapping} action="limpiar mapeo" variant="secondary">Limpiar</PermissionButton>
          </div>
        </div>
        <div className="hw-mapping-grid">
          {Array.from({ length: 30 }, (_, i) => {
            const qKey = `Q${i + 1}`;
            const map = mapping[qKey] || {};
            return (
              <div key={qKey} className="hw-mapping-row">
                <strong>{qKey}</strong>
                <select value={map.headId || ""} onChange={(e) => updateMappingCell(qKey, "headId", e.target.value)}>
                  <option value="">(sin asignar)</option>
                  {heads.map((h) => <option key={h.id} value={h.id}>{h.id} — {h.name}</option>)}
                </select>
                <select value={map.color || ""} onChange={(e) => updateMappingCell(qKey, "color", e.target.value)} disabled={!map.headId}>
                  <option value="">color…</option>
                  {colorOptions.map((c) => <option key={c} value={c}>{colorLabels[c] || c}</option>)}
                </select>
              </div>
            );
          })}
      ...
      <Panel>
        <div className="panel-header"><div><h3>Matriz de conflictos (auto-derivada)</h3><p>Pares de salidas Q que NUNCA pueden estar ON a la vez, calculadas desde geometry.conflicts del cruce + el mapeo. El backend rechaza ticks que violen estas reglas y aplica all-off.</p></div></div>
        {conflictPairs.length === 0 ? (
          <p className="muted">Sin conflictos detectados (aún) — define al menos un mapeo en color «green» y comprueba que el cruce tenga grupos en conflicto.</p>
        ) : (
          <div className="hw-conflict-list">
            {conflictPairs.map(([a, b]) => (
              <span key={`${a}-${b}`} className="tag">Q{a} ⨯ Q{b}</span>
            ))}
          </div>
        )}
      </Panel>

      {hardwareClient.faults.length > 0 && (
        <Panel>
          <div className="panel-header"><div><h3>Alarmas recientes del backend</h3></div></div>
          <div className="hw-fault-list">
            {hardwareClient.faults.slice(0, 10).map((f, i) => (
              <div key={i} className={`hw-fault hw-fault-${f.level || "info"}`}>
                <span className="muted">{new Date(f.at).toLocaleTimeString()}</span>
                <strong>{f.level || "fault"}</strong>
                <span>{f.message}</span>
              </div>
            ))}
          </div>
...
function simulateCrossingIO(crossing, signal, settings) {
  const inputs = [];
  const outputs = [];
  if (!crossing) return { inputs, outputs, module: HYDRA_IO_MODULE };

  // DI: lazos por dirección NS y EW (modelo simplificado del proyecto)
  const directions = [
    { key: "NS", label: "Norte/Sur" },
    { key: "EW", label: "Este/Oeste" },
  ];
  let diSlot = 0;
  for (const dir of directions) {
    if (diSlot >= HYDRA_IO_MODULE.diSlots) break;
    const arrival = effectiveArrival(crossing, dir.key);
    inputs.push({
      slot: diSlot++,
      kind: "lazo",
      label: `Lazo ${dir.label}`,
      direction: dir.key,
      state: arrival > 0 && (settings?.loopsEnabled !== false) ? 1 : 0,
      detail: settings?.loopsEnabled === false ? "Lazos desactivados" : `Llegada simulada ${arrival}`,
    });
  }

  // DI: pulsadores peatonales por dirección
  for (const dir of directions) {
    if (diSlot >= HYDRA_IO_MODULE.diSlots) break;
    const requests = Number(crossing.pedestrianRequests?.[dir.key] || 0);
    inputs.push({
      slot: diSlot++,
      kind: "pulsador",
      label: `Pulsador peatonal ${dir.label}`,
      direction: dir.key,
      state: requests > 0 ? 1 : 0,
      detail: `Peticiones acumuladas: ${requests}`,
    });
  }

  // DI: selector manual / auto
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const manual = !!(crossing.operatorManual || crossing.localMode);
    inputs.push({
      slot: diSlot++,
      kind: "selector",
      label: "Selector manual/auto",
      state: manual ? 1 : 0,
      detail: crossing.operatorManual ? "MANUAL/persona" : crossing.localMode ? "Local" : "Automatico",
    });
  }

  // DI: alarmas hardware
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const ioOk = crossing.hardware?.ioModuleOk !== false;
    inputs.push({
      slot: diSlot++,
      kind: "alarma",
      label: "Modulo E/S",
      state: ioOk ? 0 : 1,
      detail: ioOk ? "OK" : "Fallo modulo E/S",
    });
  }
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const loopsOk = crossing.hardware?.loopDetectorOk !== false;
    inputs.push({
      slot: diSlot++,
      kind: "alarma",
      label: "Detector de lazos",
      state: loopsOk ? 0 : 1,
      detail: loopsOk ? "OK" : "Detector con fallo",
    });
  }
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const camOk = crossing.hardware?.cameraOk !== false;
    inputs.push({
      slot: diSlot++,
      kind: "alarma",
      label: "Linea camaras",
      state: camOk ? 0 : 1,
      detail: camOk ? "OK" : "Linea camara con fallo",
    });
  }
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const opticFault = Object.values(crossing.hardware?.opticFaults || {}).some(Boolean);
    inputs.push({
      slot: diSlot++,
      kind: "alarma",
      label: "Fallo optica",
      state: opticFault ? 1 : 0,
      detail: opticFault ? "Al menos una optica con fallo" : "Sin fallos de optica",
    });
  }
  if (diSlot < HYDRA_IO_MODULE.diSlots) {
    const voltage = Number(crossing.hardware?.voltage24 ?? 24);
    const lowVoltage = voltage < 22.5 || voltage > 26;
    inputs.push({
      slot: diSlot++,
      kind: "alarma",
      label: "Fuente 24V",
      state: lowVoltage ? 1 : 0,
      detail: `${voltage.toFixed(1)} V`,
    });
  }

  // DO: por cada cabeza, un canal por optica con su estado actual
  let doSlot = 0;
  const heads = crossing.geometry?.signalHeads || [];
  for (const head of heads) {
    const currentState = signal?.outputs?.[head.id] || "red";
    for (const optic of head.optics || []) {
      if (doSlot >= HYDRA_IO_MODULE.doSlots) break;
      outputs.push({
        slot: doSlot++,
        kind: "optica",
        label: `${head.name || head.id} · ${HYDRA_DO_OPTIC_LABELS[optic] || optic}`,
        head: head.id,
        optic,
        state: currentState === optic ? 1 : 0,
      });
    }
    if (doSlot >= HYDRA_IO_MODULE.doSlots) break;
  }

  return { inputs, outputs, module: HYDRA_IO_MODULE };
}

function IOMappingPanel({ selected, signal, settings }) {
  if (!selected) return null;
  const io = simulateCrossingIO(selected, signal, settings);
  const diActive = io.inputs.filter((row) => row.state).length;
  const doActive = io.outputs.filter((row) => row.state).length;
  return (
    <Panel className="io-panel">
      <div className="panel-header">
        <div>
          <h2>Mapa de E/S DI/DO</h2>
          <p>Simulacion del modulo {io.module.name} ({io.module.diSlots} DI / {io.module.doSlots} DO). Estado en vivo derivado del cruce {selected.id}.</p>
        </div>
        <div className="panel-tools">
          <span className="tag">DI {diActive}/{io.inputs.length} activas</span>
          <span className="tag">DO {doActive}/{io.outputs.length} encendidas</span>
          <HelpButton title="Mapa de E/S" items={HYDRA_IO_HELP} />
        </div>
      </div>
      <div className="io-grid">
        <div className="io-grid-side">
          <h3>Entradas digitales (DI)</h3>
          <ul className="io-list">
            {io.inputs.map((row) => (
              <li key={`di-${row.slot}`} className={`io-row ${row.state ? "io-on" : "io-off"}`}>
                <span className="io-slot">DI{String(row.slot).padStart(2, "0")}</span>
                <span className="io-kind">{HYDRA_DI_KIND_LABELS[row.kind] || row.kind}</span>
                <span className="io-label">{row.label}</span>
                <span className="io-state">{row.state ? "ON" : "OFF"}</span>
                {row.detail && <small className="io-detail">{row.detail}</small>}
              </li>
            ))}
            {io.inputs.length === 0 && <li className="io-row io-off"><span className="io-label">Sin entradas configuradas.</span></li>}
          </ul>
        </div>
        <div className="io-grid-side">
          <h3>Salidas digitales (DO)</h3>
          <ul className="io-list">
            {io.outputs.map((row) => (
              <li key={`do-${row.slot}`} className={`io-row ${row.state ? "io-on" : "io-off"}`}>
                <span className="io-slot">DO{String(row.slot).padStart(2, "0")}</span>
                <span className="io-kind">Optica</span>
                <span className="io-label">{row.label}</span>
                <span className="io-state">{row.state ? "ON" : "OFF"}</span>
              </li>
            ))}
            {io.outputs.length === 0 && <li className="io-row io-off"><span className="io-label">Sin salidas configuradas.</span></li>}
          </ul>
        </div>
      </div>
    </Panel>
...
function AuditPanel({ audit, activeRole, onClear, onPermissionDenied }) {
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState("todas");
  const [result, setResult] = useState("todos");
  const categories = ["todas", ...new Set(audit.map((entry) => entry.category).filter(Boolean))];
  const results = ["todos", ...new Set(audit.map((entry) => entry.result).filter(Boolean))];
  const filtered = audit.filter((entry) => {
    const text = `${entry.userName} ${entry.username} ${entry.roleName} ${entry.projectName} ${entry.category} ${entry.action} ${entry.target} ${entry.result} ${entry.permission} ${entry.detail}`.toLowerCase();
  ...
  // R30: export CSV para abrir directamente en Excel/Sheets sin pasos extra.
  function exportFilteredCsv() {
    if (typeof window === "undefined" || !filtered.length) return;
    const headers = ["id", "ts", "userName", "username", "roleName", "projectName", "category", "action", "target", "result", "permission", "detail"];
    const escape = (value) => {
    ...
    for (const entry of filtered) {
      lines.push(headers.map((key) => escape(entry[key])).join(";"));
    }
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `hydra-audit-${HYDRA_VERSION_SLUG}-${new Date().toISOString().replace(/[:.]/g, "-")}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Panel className="audit-panel">
      <div className="panel-header">
        <div>
          <h2>Auditoria estructurada</h2>
          <p>Busqueda local por persona, rol, ciudad, accion, resultado y permiso usado.</p>
        </div>
        <div className="panel-tools">
          <span className="tag">{filtered.length}/{audit.length}</span>
          <PermissionButton role={activeRole} permission="viewReports" onDenied={onPermissionDenied} onClick={exportFiltered} action="exportar auditoria JSON" variant="secondary" disabled={filtered.length === 0}>Exportar JSON</PermissionButton>
          <PermissionButton role={activeRole} permission="viewReports" onDenied={onPermissionDenied} onClick={exportFilteredCsv} action="exportar auditoria CSV" variant="secondary" disabled={filtered.length === 0}>Exportar CSV</PermissionButton>
      ...
      <div className="audit-filters">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar usuario, accion, ciudad..." />
        ...
        {filtered.map((entry) => (
          <article key={entry.id} className={`audit-entry ${entry.result}`}>
            ...
            <div>
              ...
              {entry.permission && <span>{permissionLabel(entry.permission)}</span>}
            </div>
            {entry.target && <p>{entry.target}</p>}
          </article>
        ))}
        {filtered.length === 0 && <div className="activity-limited-note"><strong>Sin resultados</strong><span>Cambia la busqueda o los filtros.</span></div>}
      </div>
    </Panel>
  );
}
...
function GeometryEditor({ selected, onUpdateGeometry }) {
  ...
  function addGroup(type, useSignalCode = false) {
    ...
    // R41 fix: auto-crear una maniobra (signal head) asociada al grupo
    // nuevo. En R40 al añadir un grupo peatonal el usuario tenía que ir
    // luego a "Maniobras semafóricas" y añadirla a mano; si lo olvidaba,
    // las fases no mostraban dropdown para ese grupo y el mapeo Q→head
    // no podía referenciarlo. Ahora viene listo de fábrica.
    const headsBase = geometry.signalHeads || defaultSignalHeadsForGeometry(geometry.geometryType, geometry.signalGroups);
    const nextHeadId = nextManeuverId(headsBase);
    const headType = defaultHeadTypeForGroupType(type);
    const newHead = makeHead(nextHeadId, `${GROUP_TYPES[type]} ${id}`, id, type, headType);
    const nextSignalHeads = [...headsBase, newHead];

    onUpdateGeometry(selected.id, { ...geometry, signalGroups: nextGroups, conflicts: nextConflicts, signalHeads: nextSignalHeads });
...
function CoordinationOverview({ crossings, corridorIds, signals, settings, onUseFirstN }) {
  ...
  const avgCycle = corridor.length
    ...
    : 0;
  const linkedMode = settings?.linkedMode !== false;

  return (
    <Panel>
      <div className="panel-header">
        <div>
          <h2>Coordinación de corredor</h2>
          <p>Onda verde y desfases coordinados entre los cruces que forman el corredor.</p>
        </div>
        <Pill className={corridor.length ? "state-green" : "state-neutral"}>{corridor.length ? "Corredor activo" : "Sin corredor"}</Pill>
      </div>

      <div className="coordination-intro">
        <p className="coordination-intro-lead">
          Esta sección coordina varios cruces como un único corredor: ordena los
          cruces en sentido geográfico, calcula el desfase de cada uno respecto
          al primero y aplica la onda verde para que un vehículo a velocidad
          objetivo encuentre los semáforos en verde. La onda verde solo surte
          efecto cuando los cruces tienen el modo enlazado activo.
        </p>

        <div className="coordination-status-grid">
          <div className="coordination-stat">
            <span>Cruces en el corredor</span>
            <strong>{corridor.length}</strong>
          </div>
          <div className="coordination-stat">
            <span>Onda verde</span>
            <strong className={greenWave >= 60 ? "ok-text" : "warn-text"}>{settingsSafeText(greenWave)} · {greenWave}%</strong>
          </div>
          <div className="coordination-stat">
            <span>Modo enlazado global</span>
            <strong className={linkedMode ? "ok-text" : "warn-text"}>{linkedMode ? "Activo" : "Desactivado"}</strong>
          </div>
          <div className="coordination-stat">
            <span>Ciclo medio</span>
            <strong>{avgCycle ? `${avgCycle} s` : "—"}</strong>
          </div>
        </div>

        {corridor.length === 0 && (
          <div className="coordination-empty-cta">
            <span>Todavía no hay cruces en el corredor. Añade cruces desde el botón “Corredor” de cada cruce o empieza con los primeros del proyecto.</span>
            <div className="coordination-empty-actions">
              <Button onClick={() => onUseFirstN(4)} variant="secondary" disabled={!crossings.length}>Usar 4 cruces</Button>
              <Button onClick={() => onUseFirstN(5)} variant="secondary" disabled={!crossings.length}>Usar 5 cruces</Button>
            </div>
          </div>
        )}
      </div>
...
function ProjectCityPanelR25({ projects, allProjects = projects, activeProjectId, activeRole, onPermissionDenied, onSwitchProject, onCreateProject, onRenameProject }) {
  const activeProject = allProjects.find((project) => project.id === activeProjectId) || projects[0] || null;
  const isSuperAdmin = activeRole?.id === "superadmin";
  return (
    <Panel className="project-city-panel">
      <div className="storage-header">
        ...
        <div className="panel-tools">
          ...
          />
          <span className="tag">{isSuperAdmin ? allProjects.length : projects.length} proyecto(s)</span>
        </div>
      </div>
      <div className="project-switcher-row">
        <label>
        ...
        <div className="project-switcher-actions">
          ...
          <PermissionButton role={activeRole} permission="manageProjectUsers" onDenied={onPermissionDenied} onClick={onRenameProject} action="renombrar proyecto activo" variant="secondary">Renombrar activo</PermissionButton>
        </div>
      </div>
      {isSuperAdmin ? (
        <div className="project-global-grid">
...
function EventsDashboardPanel({ log, audit = [], issues, activeRole }) {
  const errors = issues.filter((issue) => issue.level === "error").length;
  const warnings = issues.filter((issue) => issue.level === "warning").length;
  ...
  return (
    <Panel className="events-dashboard-panel">
      ...
      <div className="r25-kpi-grid">
        ...
        <Info title="Auditoria" value={audit.length} />
        <Info title="Errores" value={errors} />
        <Info title="Avisos" value={warnings} />
  ...
  return (
    <Panel className="system-overview-panel">
      ...
      <div className="build-info-line">
        <span><strong>Build:</strong> <code>{HYDRA_BUILD_HASH}</code></span>
        <span><strong>Fecha:</strong> <code>{HYDRA_BUILD_DATE}</code></span>
      </div>
    </Panel>
  );
}

function SectionPage({ title, description, tag, help, children }) {
  return (
    <section className="section-page">
      <div className="section-page-header">
        <div className="section-page-title">
          <h2>{title}</h2>
          {help && help.length ? <HelpButton title={title} items={help} /> : null}
        </div>
        {tag && <span className="tag">{tag}</span>}
        {!help && description ? <p className="section-page-description">{description}</p> : null}
      </div>
...
function SessionMenuR26({ user, role, project, onOpenUsers, onSwitchUser, onLogout }) {
  return (
    <div className="session-menu">
      <div className="session-menu-header">
        <span>Sesion activa</span>
...
function LoginModalR26({ open, usernameInput, pin, password, error, onUsernameChange, onPinChange, onPasswordChange, onSubmit, onClose }) {
  if (!open) return null;
  return (
    <div className="access-modal-backdrop" role="presentation">
      <div className="access-modal" role="dialog" aria-modal="true" aria-label="Acceso Hydra">
        ...
        <form className="access-form" onSubmit={onSubmit}>
          ...
          <label>
            ...
            <input
              ...
              placeholder="Solo si tu usuario tiene contraseña"
        ...
        <p className="access-warning">Acceso local simulado. La seguridad real (hash, MFA y sesiones multi-PC) llega con backend en V9.</p>
      </div>
    </div>
  );
}
        ...
        <form className="access-form" onSubmit={onSubmit}>
          ...
          <label>
            ...
            <input
              ...
              pattern="[0-9]*"
              value={draft.pin}
              onChange={(event) => onChange("pin", event.target.value.replace(/[^0-9]/g, ""))}
              placeholder="Solo dígitos (p. ej. 1234)"
              autoComplete="off"
              required
            />
          </label>
          <label>
            ...
            <input
              ...
              placeholder="Opcional: si la pones, se pedirá al entrar"
...
function UsersPanelR26({
  ...
  onEditUser,
  onToggleUser,
  onToggleProjectForUser,
  onEditProjectPermissions,
  onDeleteUser,
}) {
  const permissionKeys = Object.keys(HYDRA_PERMISSION_LABELS);
  const isSuperadmin = activeRole?.id === "superadmin";
  ...
  return (
    <Panel className="users-panel">
      <div className="users-panel-header">
        <div>
          <h2>Usuarios personales y roles</h2>
          <p>R26 separa persona, rol, PIN demo, ciudades asignadas y permisos por proyecto.</p>
        </div>
        <div className="panel-tools">
          <span className="tag">{activePerson ? userDisplay(activePerson, activeRole) : "Sin sesion"}</span>
          <Button onClick={onCreateUser} disabled={!canManageAny} variant="secondary">Crear usuario</Button>
        </div>
      </div>

      <div className="user-session-strip">
      ...
      <div className="users-table-wrap">
        <table className="permissions-table users-table">
          <thead>
            <tr>
              <th>Persona</th>
              <th>Usuario</th>
          ...
          <tbody>
            {visibleUsers.map((user) => {
              ...
              const assigned = user.projectIds?.includes("*")
                ? "Todas"
                : projects.filter((project) => user.projectIds?.includes(project.id)).map((project) => project.name).join(", ") || "Sin ciudad";
              const manageable = canManageTargetUser(activePerson, activeRole, user, activeProject?.id);
              return (
                <tr key={user.id}>
                  <td><strong>{user.name}</strong></td>
                  <td>{user.username}</td>
                  <td>{role?.name || user.roleId}</td>
                  <td>{assigned}</td>
                  <td>{user.active === false ? "Inactivo" : "Activo"}</td>
                  <td>
                    <div className="inline-action-row">
                      <button type="button" onClick={() => onRequestLogin(user.id)}>Entrar</button>
                      <button type="button" disabled={!manageable} onClick={() => onEditUser(user.id)}>Editar</button>
                      <button type="button" disabled={!manageable} onClick={() => onToggleProjectForUser(user.id)}>{userCanAccessProject(user, activeProject?.id) ? "Quitar ciudad" : "Asignar ciudad"}</button>
                      <button type="button" disabled={!manageable} onClick={() => onEditProjectPermissions(user.id)}>Permisos ciudad</button>
                      <button type="button" disabled={!manageable} onClick={() => onToggleUser(user.id)}>{user.active === false ? "Activar" : "Desactivar"}</button>
                      <button type="button" className="danger" disabled={!manageable || user.id === activePerson?.id} onClick={() => onDeleteUser(user.id)}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              );
            ...
            {visibleUsers.length === 0 && (
              <tr>
                <td colSpan="6">No hay usuarios visibles para esta ciudad.</td>
              </tr>
            )}
          </tbody>
        </table>
        {!canManageAny && (
          <div className="activity-limited-note">
            ...
            <span>Para modificar usuarios necesitas Administrar usuarios global o Administrar usuarios del proyecto.</span>
          </div>
        )}
      </div>
      ...
      <div className="system-activity-panel">
        <div className="system-activity-header">
          <div>
            <h3>Actividad del sistema</h3>
            <p>{isSuperadmin ? "Vista local simulada para presencia, auditoria y actividad multiusuario." : "Este rol solo ve su sesion y la actividad permitida dentro de su ciudad."}</p>
          </div>
          <span className={isSuperadmin ? "activity-access full" : "activity-access limited"}>
            {isSuperadmin ? "Vista completa" : "Vista limitada"}
          </span>
        ...
        <div className="activity-presence-grid">
          {visibleActivity.map((item, index) => {
            const role = getRoleById(item.roleId);
            const user = item.userId ? getUserById(users, item.userId) : null;
            return (
              <article key={`${item.userId || item.roleId}-${index}`} className={`presence-card status-${item.status}`}>
                <div className="presence-card-top">
                  <div>
                    <strong>{user?.name || role?.name || "Usuario"}</strong>
                    <span>{item.summary}</span>
                  </div>
                  <em>{item.status}</em>
                </div>
        ...
        {isSuperadmin ? (
          ...
          <div className="activity-limited-note">
            <strong>Permiso limitado</strong>
            <span>La actividad global de otros usuarios queda reservada al Superadministrador.</span>
          </div>
        )}
      </div>
    </Panel>
...
function App() {
  ...
  const savedUsersOnLoad = useMemo(() => loadHydraUsers(savedProjectsOnLoad.projects), [savedProjectsOnLoad.projects]);
  ...
  const [users, setUsers] = useState(savedUsersOnLoad);
  const [loginUsernameInput, setLoginUsernameInput] = useState("");
  const [loginPin, setLoginPin] = useState("");
  const [loginPassword, setLoginPassword] = useState("");
  const [loginError, setLoginError] = useState("");
  const [userFormState, setUserFormState] = useState({ open: false, mode: "create", userId: null, draft: { name: "", username: "", roleId: "viewer", pin: "1111", password: "", active: true }, error: "" });
  const [permissionsModalState, setPermissionsModalState] = useState({ open: false, userId: null, draft: [] });
  // R31: tab activa dentro de la seccion Cruces (split list + tabs).
  const [crucesTab, setCrucesTab] = useState("control");

  // R30: deteccion de cambio de revision. Si la ultima cargada en este
  // navegador no es la actual, mostramos dialogo para ofrecer reset.
  const previousVersionDetected = useMemo(() => {
    if (typeof window === "undefined" || !storageAvailable()) return null;
    try {
      return window.localStorage.getItem(HYDRA_VERSION_LAST_KEY);
    } catch {
      return null;
    }
  }, []);
  const [revisionResetOpen, setRevisionResetOpen] = useState(() => previousVersionDetected !== HYDRA_VERSION_LABEL);

  function handleRevisionReset() {
    if (typeof window !== "undefined" && storageAvailable()) {
      [
        HYDRA_STORAGE_KEY,
        HYDRA_PROJECTS_KEY,
      ...
      window.localStorage.setItem(HYDRA_VERSION_LAST_KEY, HYDRA_VERSION_LABEL);
      window.location.reload();
    }
  }

  function handleRevisionKeep() {
    if (typeof window !== "undefined" && storageAvailable()) {
      window.localStorage.setItem(HYDRA_VERSION_LAST_KEY, HYDRA_VERSION_LABEL);
    }
    setRevisionResetOpen(false);
    addLog(`Revision actualizada de ${previousVersionDetected || "ninguna"} a ${HYDRA_VERSION_LABEL}, datos previos conservados.`, { category: "sistema", action: "actualizar revision", target: HYDRA_VERSION_LABEL });
  }
  ...
  const [hardwareConfig, setHardwareConfig] = useState(() => normalizeHardwareConfig(activeProjectOnLoad?.hardware));
  const hardwareClient = useHardwareClient(hardwareConfig.backendBaseUrl || "");

  const selected = crossings.find((c) => c.id === selectedId) || null;
  const activeProject = projects.find((project) => project.id === activeProjectId) || projects[0] || null;
  const activePerson = getUserById(users, activeUser?.userId) || getFirstUserForRole(users, activeUser?.roleId);
  const activeBaseRole = getRoleById(activePerson?.roleId || activeUser?.roleId);
  const activeRole = activeBaseRole ? { ...activeBaseRole, permissions: effectivePermissionsForUser(activePerson, activeProjectId, activeBaseRole) } : null;
  const visibleProjects = visibleProjectsForUser(projects, activePerson);
  ...
  }, [activePerson?.id, activeProjectId, projects]);
  ...
  function submitLogin(event) {
    ...
    const typed = String(loginUsernameInput || "").trim();
    if (!typed) {
      setLoginError("Escribe un usuario.");
      return;
    }
    const user = users.find((u) => u.username?.toLowerCase() === typed.toLowerCase()) ||
                 users.find((u) => u.name?.toLowerCase() === typed.toLowerCase());
    if (!user || user.active === false) {
      setLoginError("Usuario o contrasena incorrectos.");
      return;
    }
    const role = getRoleById(user.roleId);
    // R30: stub MFA — solo se valida PIN en cliente; si el usuario tiene
    // requiresMFA, en R30 no pedimos los factores extra (UI llegara con
    // backend real en V9). De momento aceptamos PIN OK + flag MFA.
    const result = verifyCredentials(user, loginPin, loginPassword, "", "");
    if (!result.ok) {
      // R32: mensaje generico para no revelar si el usuario existe.
      // Excepcion: mfa-missing es informativo (ya pasaste el PIN).
      const message = result.reason === "mfa-missing"
        ? "Falta factor adicional (MFA llega con backend en V9)."
    ...
    setLoginPassword("");
    setLoginError("");
    setLog((old) => [`Sesion iniciada: ${user.name} (${role.name}).`, ...old].slice(0, 120));
    setAudit((old) => [makeAuditEntryFor(user, role, activeProject, "acceso", "inicio de sesion", user.username, "ok", "viewSystem"), ...old].slice(0, 400));
  }
  ...
  function logout(reason = "manual") {
    ...
    setLoginPassword("");
    setLoginError("");
    setLog((old) => [reason === "inactivity" ? "Sesion cerrada por inactividad." : "Sesion cerrada.", ...old].slice(0, 120));
    if (user && role) setAudit((old) => [makeAuditEntryFor(user, role, activeProject, "acceso", reason === "inactivity" ? "cierre por inactividad" : "cierre de sesion", user.username, "ok", "viewSystem"), ...old].slice(0, 400));
  }

  // R30: hook de inactividad. Activo solo cuando hay sesion abierta.
  const { warning: inactivityWarning, dismissWarning: dismissInactivityWarning } = useInactivityTimeout({
    active: !!activeUser,
    totalMs: HYDRA_INACTIVITY_TOTAL_MS,
    warns: HYDRA_INACTIVITY_WARN_AT,
  ...
  }, [crossings, manualLinks, tick, selectedId, corridorIds, settings, log, audit, hardwareConfig, activeProjectId, activeProject?.name]);

  // R40: refs para que el setInterval del bucle de fases NO se recree cada
  // tick. En R39 los deps incluían `tick`, lo que reiniciaba el interval cada
  // 1s y generaba gaps grandes (>3s) cuando el browser throttleaba la
  // pestaña → disparos repetidos de safe-mode. Ahora el interval se crea una
  // sola vez al cambiar `running` y lee del ref los valores frescos.
  const tickRef = useRef(tick);
  const settingsRef = useRef(settings);
  const hardwareConfigRef = useRef(hardwareConfig);
  const selectedRef = useRef(selected);
  const sendTickRef = useRef(hardwareClient.sendTick);
  useEffect(() => { tickRef.current = tick; }, [tick]);
  useEffect(() => { settingsRef.current = settings; }, [settings]);
  useEffect(() => { hardwareConfigRef.current = hardwareConfig; }, [hardwareConfig]);
  useEffect(() => { selectedRef.current = selected; }, [selected]);
  useEffect(() => { sendTickRef.current = hardwareClient.sendTick; }, [hardwareClient.sendTick]);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const currentHwConfig = hardwareConfigRef.current;
      const currentSelected = selectedRef.current;

      // R39/R40: emite tick al backend Modbus para el cruce activo si el
      // control hardware está habilitado y hay un mapping definido. El
      // backend traduce headStates → relés físicos vía Modbus TCP (o
      // simulador). Usamos refs para no recrear el setInterval cada tick.
      if (currentHwConfig.modbusEnabled && currentSelected) {
        try {
          const sig = getSignal(currentSelected, currentTick, currentSettings);
          const mapping = (currentHwConfig.mappings || {})[currentSelected.id];
          if (mapping) {
            const targetRelays = buildTargetRelays(mapping, sig.headStates);
            sendTickRef.current(currentSelected.id, targetRelays);
          }
        } catch (e) { /* swallow: hardware no debe romper la simulación */ }
      }

      setCrossings((old) => old.map((crossing) => {
        const signal = getSignal(crossing, currentTick, currentSettings);
        const queues = { ...crossing.queues };
        const pedestrianRequests = { ...crossing.pedestrianRequests };

        if (currentSettings.autoTraffic) {
        ...
        if (signal.stage === "green") {
          queues.NS = clamp(queues.NS - currentSettings.dischargeRate * 0.5, 0, MAX_QUEUE);
          queues.EW = clamp(queues.EW - currentSettings.dischargeRate * 0.5, 0, MAX_QUEUE);
  ...
  }, [running]);  // R40: deps solo `running` — el setInterval se crea 1 sola vez al arrancar/parar
  ...
  function handlePermissionDenied(permission, action = "accion") {
    const message = `Bloqueado: ${action} requiere ${permissionLabel(permission)} (${activePerson?.name || activeRole?.name || "sin sesion"}).`;
    setLog((old) => [`${new Date().toLocaleTimeString()} - ${message}`, ...old].slice(0, 80));
    setAudit((old) => [
      makeAuditEntryFor(activePerson, activeRole, activeProject, "permisos", action, permissionLabel(permission), "bloqueado", permission),
      ...old,
    ].slice(0, 400));
    setEventsOpen(true);
  ...
  function updateHardwareConfig(next) {
    setHardwareConfig(normalizeHardwareConfig(next));
  }

  function updateMapping(crossingId, nextMapping) {
    if (!crossingId) return;
    setHardwareConfig((prev) => ({
      ...prev,
      mappings: { ...(prev.mappings || {}), [crossingId]: nextMapping || {} },
    }));
  }
  ...
  function applyProject(project, message = "") {
    ...
    setHardwareConfig(normalizeHardwareConfig(snapshot.hardware));
    setLastCheck([]);
    setLinkStartId(null);
    setLinkMode(false);
  }
  ...
  function switchProject(projectId) {
    ...
    if (activePerson && !userCanAccessProject(activePerson, projectId)) {
      handlePermissionDenied("viewSystem", "cambiar a ciudad no asignada");
      return;
    }
  ...
  function createUser() {
    ...
    setUserFormState({
      ...
      mode: "create",
      userId: null,
      draft: { name: "Nuevo usuario", username: `usuario${users.length + 1}`, roleId: allowed[0]?.id || "viewer", pin: "1111", password: "", active: true },
      error: "",
    });
  }
  ...
  function editUser(userId) {
    ...
    if (!canManageTargetUser(activePerson, activeRole, target, activeProjectId)) {
    ...
    setUserFormState({
      ...
      mode: "edit",
      userId,
      draft: { name: target.name, username: target.username, roleId: target.roleId, pin: target.pin, password: target.password || "", active: target.active !== false },
      error: "",
    });
  }
  ...
  function submitUserForm(event) {
    ...
    if (!cleanPin) {
      setUserFormState((state) => ({ ...state, error: "El PIN no puede estar vacio." }));
      return;
    }
    if (!/^[0-9]+$/.test(cleanPin)) {
      setUserFormState((state) => ({ ...state, error: "El PIN solo puede contener numeros." }));
      return;
    }
    // R35: contraseña libre y opcional (letras, números y símbolos).
    const cleanPassword = String(draft.password || "");
  ...
  function toggleUser(userId) {
    const target = getUserById(users, userId);
    if (!canManageTargetUser(activePerson, activeRole, target, activeProjectId)) {
      handlePermissionDenied("manageProjectUsers", "activar o desactivar usuario");
      return;
    }
    if (target.roleId === "superadmin" && target.active !== false) {
      const activeSuperadmins = users.filter((user) => user.roleId === "superadmin" && user.active !== false).length;
      if (activeSuperadmins <= 1) {
        alert("No se puede desactivar el ultimo Superadministrador activo.");
        addLog("Bloqueado: intento de desactivar el ultimo Superadministrador.", { category: "permisos", action: "desactivar usuario", target: target.username, result: "bloqueado", permission: "manageUsers" });
        return;
      }
    }
    setUsers((old) => old.map((user) => user.id === userId ? { ...user, active: user.active === false } : user));
    addLog(`${target.active === false ? "Activado" : "Desactivado"} usuario: ${target.username}.`, { category: "permisos", action: target.active === false ? "activar usuario" : "desactivar usuario", target: target.username, permission: roleHasPermission(activeRole, "manageUsers") ? "manageUsers" : "manageProjectUsers" });
  }
  ...
  function deleteUser(userId) {
    ...
    if (!target) return;
    if (!canManageTargetUser(activePerson, activeRole, target, activeProjectId)) {
      handlePermissionDenied("manageProjectUsers", "eliminar usuario");
      return;
    }
    if (target.id === activePerson?.id) {
      alert("No puedes eliminar tu propio usuario mientras tengas la sesion abierta.");
      addLog("Bloqueado: intento de eliminar el usuario activo.", { category: "permisos", action: "eliminar usuario", target: target.username, result: "bloqueado", permission: "manageUsers" });
      return;
    }
    if (target.roleId === "superadmin") {
      const activeSuperadmins = users.filter((user) => user.roleId === "superadmin" && user.active !== false).length;
      const targetCounted = target.active !== false ? 1 : 0;
      if (activeSuperadmins - targetCounted <= 0) {
        alert("No se puede eliminar el ultimo Superadministrador activo.");
        addLog("Bloqueado: intento de eliminar el ultimo Superadministrador.", { category: "permisos", action: "eliminar usuario", target: target.username, result: "bloqueado", permission: "manageUsers" });
        return;
      }
    }
    if (!window.confirm(`Eliminar el usuario ${target.name} (${target.username})? Esta accion no se puede deshacer.`)) {
      addLog(`Cancelada eliminacion del usuario ${target.username}.`, { category: "permisos", action: "eliminar usuario", target: target.username, result: "cancelado", permission: roleHasPermission(activeRole, "manageUsers") ? "manageUsers" : "manageProjectUsers" });
      return;
    }
    setUsers((old) => old.filter((user) => user.id !== userId));
    addLog(`Usuario eliminado: ${target.name} (${target.username}).`, { category: "permisos", action: "eliminar usuario", target: target.username, permission: roleHasPermission(activeRole, "manageUsers") ? "manageUsers" : "manageProjectUsers" });
  }

  function toggleProjectForUser(userId) {
    const target = getUserById(users, userId);
    if (!canManageTargetUser(activePerson, activeRole, target, activeProjectId)) {
      handlePermissionDenied("manageProjectUsers", "asignar ciudad a usuario");
      return;
    }
    const hasProject = userCanAccessProject(target, activeProjectId);
    setUsers((old) => old.map((user) => {
      if (user.id !== userId || user.projectIds?.includes("*")) return user;
      return {
        ...user,
        projectIds: hasProject ? user.projectIds.filter((id) => id !== activeProjectId) : [...new Set([...(user.projectIds || []), activeProjectId])],
      };
    }));
    addLog(`${hasProject ? "Retirada" : "Asignada"} ciudad ${activeProject?.name || ""} a ${target.username}.`, { category: "permisos", action: hasProject ? "quitar ciudad" : "asignar ciudad", target: target.username, permission: roleHasPermission(activeRole, "manageUsers") ? "manageUsers" : "manageProjectUsers" });
  }

  function editProjectPermissions(userId) {
    const target = getUserById(users, userId);
    if (!canManageTargetUser(activePerson, activeRole, target, activeProjectId)) {
      handlePermissionDenied("manageProjectUsers", "conceder permisos de ciudad");
      return;
    }
  ...
  function renderSectionContent() {
    if (activeSection === "Mapa del sistema") {
      return (
        <SectionPage title="Mapa del sistema" help={[
          "Mapa operativo: ubicación de cruces y enlaces sobre OpenStreetMap.",
          "Centro de Control: regulador, modos manual/local/averia, intervencion segura del cruce activo.",
          "Cruces seleccionados: corredor activo y onda verde.",
          "Modo añadir: pulsa en el mapa para colocar cruce nuevo.",
          "Selecciona dos marcadores para crear o quitar enlace manual."
        ]}>
          {renderMapAndControl()}
          {renderCorridor()}
        </SectionPage>
    ...
    if (activeSection === "Cruces") {
      const tabs = [
        ...
        { id: "hardware", label: "Hardware y E/S" },
        { id: "geometria", label: "Geometría" },
      ];
      return (
        <SectionPage title="Cruces" help={[
          "Ficha completa del cruce seleccionado en pestañas: control, demanda, hardware y geometría.",
          "La lista de la izquierda permite cambiar de cruce sin volver al mapa.",
          "Operacion del regulador y modos manual/local/averia requieren operateCrossings.",
          "Editar geometria/fases/conflictos requiere editGeometry."
        ]} tag={selected?.name || "Sin selección"}>
          <div className="cruces-split">
            <aside className="cruces-list">
              <div className="cruces-list-header">
                <strong>Cruces ({crossings.length})</strong>
                <button type="button" className="cruces-list-add" disabled={!roleHasPermission(activeRole, "editGeometry")} onClick={guardAction("editGeometry", "anadir cruce desde lista", () => addCrossing(CENTER[0], CENTER[1]))}>+</button>
              </div>
              <ul>
                {crossings.map((c) => (
                  <li key={c.id}>
                    <button type="button" className={c.id === selected?.id ? "active" : ""} onClick={() => setSelectedId(c.id)}>
                      <span className="cruces-list-id">{c.id}</span>
                      <span className="cruces-list-name">{c.name}</span>
                      <span className={`cruces-list-mode mode-${c.operatorManual ? "manual" : c.localMode ? "local" : "auto"}`}>
                        {c.operatorManual ? "Manual" : c.localMode ? "Local" : "Auto"}
                      </span>
                    </button>
                  </li>
                ))}
                {crossings.length === 0 && <li className="cruces-list-empty">Sin cruces. Pulsa + para añadir.</li>}
              </ul>
            </aside>
            <main className="cruces-detail">
              {selected && selectedSignal ? (
                <>
                  <nav className="cruces-tabs">
                    {tabs.map((tab) => (
                      <button key={tab.id} type="button" className={tab.id === crucesTab ? "active" : ""} onClick={() => setCrucesTab(tab.id)}>{tab.label}</button>
                    ))}
                  </nav>
                  {crucesTab === "control" && (
                    <>
                      ...
                      <ControlPanel
                        ...
                        onSetLinkMode={guardAction("changePlans", "cambiar modo unir cruces", (value) => { setLinkMode(value); setLinkStartId(null); })}
                        onClearLinkStart={() => setLinkStartId(null)}
                        onRemoveAllLinks={guardAction("changePlans", "borrar enlaces manuales", () => { setManualLinks([]); addLog("Enlaces manuales borrados.", { category: "corredor" }); })}
                      />
                      <DecisionPanel selected={selected} signal={selectedSignal} settings={settings} />
                    </>
                  )}
                  ...
                  {crucesTab === "hardware" && (
                    <>
                      <HardwareHealthPanel selected={selected} onToggleOpticFault={guardAction("maintainHardware", "cambiar estado de optica", toggleOpticFault)} onToggleHardwareFlag={guardAction("maintainHardware", "cambiar estado de hardware", toggleHardwareFlag)} />
                      <IOMappingPanel selected={selected} signal={selectedSignal} settings={settings} />
                    </>
                  )}
                  {crucesTab === "geometria" && (
                    <GeometryEditor selected={selected} onUpdateGeometry={guardAction("editGeometry", "editar geometria avanzada", updateGeometry)} />
    ...
    if (activeSection === "Coordinación") {
      return (
        <SectionPage title="Coordinación" help={[
          "Selecciona cruces para componer el corredor en orden geografico.",
          "Cada cruce muestra su desfase y banda verde respecto al inicio.",
          "Onda verde solo se aplica si los cruces tienen modo enlazado activo.",
          "Cambiar planes y desfases requiere permiso changePlans."
        ]}>
    ...
    if (activeSection === "Escenarios") {
      return (
        <SectionPage title="Escenarios" help={[
          "Plantillas de planes para horarios punta, valle, nocturno o eventos.",
          "Cambiar el escenario activo aplica los multiplicadores de demanda y los parametros del cruce.",
          "Los escenarios afectan a todos los cruces del proyecto activo."
        ]}>
          <SectionPlaceholder
    ...
    if (activeSection === "Informes") {
      return (
        <SectionPage title="Informes" help={[
          "Resumen de validacion: incidencias activas, conflictos y avisos del proyecto.",
          "Exporta el estado actual en JSON para llevartelo a otra maquina o sesion.",
          "Las incidencias estan separadas por nivel: error, aviso y informacion."
        ]}>
    ...
    if (activeSection === "Eventos") {
      return (
        <SectionPage title="Eventos" help={[
          "Registro vivo en formato timeline: cada accion se ve como una tarjeta con su categoria y resultado.",
          "Auditoria estructurada y log breve en panel lateral derecho.",
          "Filtros y exports siguen viviendo en el panel de auditoria."
        ]}>
          <div className="eventos-timeline-layout">
            <div className="eventos-timeline-main">
              <EventsDashboardPanel log={log} audit={audit} issues={lastCheck.length ? lastCheck : liveIssues} activeRole={activeRole} />
              <Panel className="eventos-feed-panel">
                <div className="panel-header">
                  <div className="section-page-title">
                    <h2>Timeline de eventos</h2>
                    <HelpButton title="Timeline" items={[
                      "Cada tarjeta es una entrada de auditoria con su hora, categoria, accion y resultado.",
                      "Las entradas con resultado bloqueado salen marcadas en rojo.",
                      "Los eventos del log breve aparecen en la columna lateral.",
                    ]} />
                  </div>
                  <span className="tag">{audit.length} entradas</span>
                </div>
                <ol className="eventos-feed">
                  {audit.length === 0 && <li className="eventos-feed-empty">No hay eventos auditados todavia.</li>}
                  {audit.map((entry) => (
                    <li key={entry.id} className={`eventos-feed-item result-${entry.result || "ok"} category-${entry.category || "sistema"}`}>
                      <div className="eventos-feed-time">{formatHydraTime(entry.ts)}</div>
                      <div className="eventos-feed-body">
                        <div className="eventos-feed-head">
                          <strong>{entry.action || "Accion"}</strong>
                          <span className={`eventos-feed-cat`}>{entry.category || "sistema"}</span>
                          {entry.result && entry.result !== "ok" && <span className={`eventos-feed-result result-${entry.result}`}>{entry.result}</span>}
                        </div>
                        <div className="eventos-feed-meta">
                          <span>{entry.userName || "Sin usuario"}</span>
                          <span>·</span>
                          <span>{entry.roleName || "Rol"}</span>
                          <span>·</span>
                          <span>{entry.projectName || "Proyecto"}</span>
                          {entry.permission && <><span>·</span><span>{permissionLabel(entry.permission)}</span></>}
                        </div>
                        {entry.target && <div className="eventos-feed-target">{entry.target}</div>}
                        {entry.detail && <div className="eventos-feed-detail">{entry.detail}</div>}
                      </div>
                    </li>
                  ))}
                </ol>
            ...
            <aside className="eventos-timeline-side">
    ...
    if (activeSection === "Dispositivos") {
      return (
        <SectionPage title="Dispositivos" help={[
          "Estado del mini PC, fuente 24V, modulo de E/S, detector de lazos y opticas del cruce activo.",
          "Mapa de E/S DI/DO simula un Advantech ADAM-6052 con 16 entradas y 16 salidas.",
          "Cambiar fallos de hardware o de optica solo lo permite el rol con maintainHardware."
        ]}>
          {selected ? (
            <>
              <HardwareHealthPanel selected={selected} onToggleOpticFault={guardAction("maintainHardware", "cambiar estado de optica", toggleOpticFault)} onToggleHardwareFlag={guardAction("maintainHardware", "cambiar estado de hardware", toggleHardwareFlag)} />
    ...
    if (activeSection === "Hardware") {
      return (
        <SectionPage title="Hardware" help={[
          "Control fisico de los semaforos del cruce activo via modulo Modbus TCP Waveshare 30CH.",
          "Modo SIMULACION: el backend usa un modulo falso interno, sin red ni hardware real. Ideal para desarrollo.",
          "Modo REAL: backend conecta al modulo en 192.168.1.44:502 y manda las ordenes a los reles.",
          "Empezar envio de fases: el frontend transmite cada segundo el estado deseado de los 30 reles al backend.",
          "El backend usa Flash ON con duracion 5s: cada rele se apaga solo si el backend se cae (watchdog distribuido).",
          "Conflict matrix automatica: derivada de geometry.conflicts + mapeo. Si dos reles incompatibles intentan estar ON a la vez -> all-off + alarma.",
          "Requiere permiso maintainHardware para todas las acciones."
        ]}>
          {selected ? (
            <HardwarePanel
              selected={selected}
              activeProject={activeProject ? { ...activeProject, hardware: hardwareConfig } : null}
              hardwareClient={hardwareClient}
              onUpdateHardwareConfig={updateHardwareConfig}
    ...
    if (activeSection === "Cámaras") {
      return (
        <SectionPage title="Cámaras" help={[
          "Camaras asociadas al cruce activo, con vision simulada por direccion.",
          "Anadir o quitar camaras y cambiar su estado requiere maintainHardware.",
          "El calculo adaptativo solo usa camaras activas y sin fallo."
        ]}>
          {selected ? (
            <CameraPanel
              selected={selected}
    ...
    if (activeSection === "Configuración") {
      return (
        <SectionPage title="Configuración" help={[
          "Ajustes globales: adaptativo, modo enlazado, lazos, camaras y manual asistido.",
          "Guardado local: punto manual, restaurar el ultimo y borrar el guardado.",
          "Cada accion respeta el permiso del rol activo y queda en auditoria."
        ]}>
    ...
    if (activeSection === "Usuarios") {
      return (
        <SectionPage title="Usuarios" help={[
          "Personas separadas de roles: cada usuario tiene su PIN demo y ciudades asignadas.",
          "Filtra la tabla por rol, ciudad o estado activo/inactivo.",
          "Crear/editar usuario abre formulario; permisos extra por ciudad usan checkboxes.",
          "El ultimo Superadministrador activo no se puede desactivar."
        ]} tag={activePerson?.name || activeRole?.name || "Sin sesión"}>
          <UsersPanelR26
            activeRole={activeRole}
            ...
            onToggleProjectForUser={toggleProjectForUser}
            onEditProjectPermissions={editProjectPermissions}
    ...
    if (activeSection === "Sistema") {
      return (
        <SectionPage title="Sistema" help={[
          "Resumen tecnico local: version, proyecto activo, persistencia y estado del servicio.",
          "Limpiar el registro o el guardado local solo lo permite el rol manageUsers.",
          "En V9 esta seccion mostrara el estado del servicio local del regulador."
        ]}>
  ...
  return (
    <main>
      <div className="hydra-shell">
            ...
            <div className="header-user-wrap">
              ...
              {sessionMenuOpen && (
                <SessionMenuR26
                  ...
                  onSwitchUser={() => openLogin(activePerson?.id || "")}
                  onLogout={logout}
                />
              )}
            </div>
        ...
        <LoginModalR26
          open={loginOpen}
          usernameInput={loginUsernameInput}
          pin={loginPin}
          password={loginPassword}
          error={loginError}
          onUsernameChange={(value) => { setLoginUsernameInput(value); setLoginError(""); }}
          onPinChange={(value) => { setLoginPin(value); setLoginError(""); }}
          onPasswordChange={(value) => { setLoginPassword(value); setLoginError(""); }}
        ...
        />
        <InactivityWarningModal warning={inactivityWarning} onDismiss={dismissInactivityWarning} />
        <RevisionResetDialog
          open={revisionResetOpen}
          previousVersion={previousVersionDetected}
          currentVersion={HYDRA_VERSION_LABEL}
          onReset={handleRevisionReset}
          onKeep={handleRevisionKeep}
        />
    </main>
  );
}

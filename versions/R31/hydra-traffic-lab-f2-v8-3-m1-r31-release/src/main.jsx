...
const HYDRA_VERSION_LABEL = "F2-V8.3-M1-R31";
const HYDRA_VERSION_SLUG = "f2-v8-3-m1-r31-release";
...
function makeDefaultHydraUsers(projects = []) {
  // R30: solo una cuenta por defecto. Las demas personas se crean desde el panel.
  // Los 8 demo de R29 se eliminaron: roles != usuarios.
  return [
    {
      id: "user-superadmin-hydra",
      name: "Superadministrador Hydra",
      username: "superadmin@hydra",
      roleId: "superadmin",
      pin: "9999",
      projectIds: ["*"],
      active: true,
...
// R30: hook de inactividad con avisos progresivos.
// activo=false desactiva el hook (no hay sesion). totalMs es el total
// hasta cerrar la sesion. warns[] son momentos en los que disparar avisos
// (kind: "soft" se cierra al interactuar, kind: "countdown" se queda
// fijo con cuenta atras).
function useInactivityTimeout({ active, totalMs, warns, onExpire }) {
  const [lastActivity, setLastActivity] = useState(() => Date.now());
  const [warning, setWarning] = useState(null); // { kind, label, remainingMs }
  ...
  useEffect(() => {
    ...
    setWarning({ ...triggered, remainingMs: remaining });
  }, [active, now, lastActivity, totalMs, warns, onExpire]);
...
function LoginModalR26({ open, users, projects, selectedUserId, pin, error, onUserChange, onPinChange, onSubmit, onClose }) {
  if (!open) return null;
  const activeUsers = users.filter((user) => user.active !== false);
  const selectedUser = getUserById(users, selectedUserId) || activeUsers[0] || users[0];
  const selectedRole = getRoleById(selectedUser?.roleId) || HYDRA_ROLES[1];
  const assignedProjects = selectedUser?.projectIds?.includes("*")
    ? "Todas las ciudades"
    : projects.filter((project) => selectedUser?.projectIds?.includes(project.id)).map((project) => project.name).join(", ") || "Sin ciudad asignada";
  return (
    <div className="access-modal-backdrop" role="presentation">
      <div className="access-modal" role="dialog" aria-modal="true" aria-label="Acceso Hydra">
        <div className="access-modal-header">
        ...
        <form className="access-form" onSubmit={onSubmit}>
          ...
          <label>
            ...
            <input
              ...
              value={pin}
              onChange={(event) => onPinChange(event.target.value)}
              placeholder="PIN o contrasena local"
              autoFocus
            />
          </label>
          {error && <div className="access-error">{error}</div>}
        ...
        <p className="access-warning">R26 separa persona, rol, ciudad asignada y auditoria. Sigue siendo acceso local/simulado; la seguridad real queda para backend, hash de contrasenas y sesiones multi-PC.</p>
      </div>
    </div>
  );
}
...
function App() {
  ...
  function openLogin(userId = activePerson?.id || users.find((user) => user.active !== false)?.id || "") {
  ...
  function submitLogin(event) {
    ...
    const user = getUserById(users, loginUserId);
    if (!user || user.active === false) {
      setLoginError("Usuario no disponible o inactivo.");
      return;
    }
    const role = getRoleById(user.roleId);
    ...
    const result = verifyCredentials(user, loginPin, "", "", "");
    if (!result.ok) {
      const message =
        result.reason === "pin" ? "PIN incorrecto." :
        result.reason === "inactive" ? "Usuario inactivo." :
        result.reason === "mfa-missing" ? "Falta factor adicional (V9)." :
  ...
  function renderSectionContent() {
    if (activeSection === "Mapa del sistema") {
      return (
        <SectionPage title="Mapa del sistema" help={[
          ...
          "Marcadores compactos con icono por tipo de grupo y lampara de color.",
          "Modo añadir desde el panel: pulsa en el mapa para colocar cruce.",
          "Selecciona dos marcadores para crear o quitar enlace manual.",
          "Para operar el cruce activo abre la seccion Cruces."
        ]}>
          <div className="map-fullview-wrap">
            <div className="map-fullview-leaflet">
              <LeafletMapView
                crossings={crossings}
                signals={signals}
                selectedId={selected?.id || ""}
            ...
            {selected ? (
              <div className="map-fullview-hud">
                <span className="map-fullview-hud-tag">Cruce activo</span>
                <h3>{selected.id} · {selected.name}</h3>
                <ul>
                  <li><strong>Modo:</strong> {selected.operatorManual ? "Manual" : selected.localMode ? "Local" : "Auto enlazado"}</li>
                  <li><strong>Geometria:</strong> {geometryLabel(selected.geometry)}</li>
                  <li><strong>Camaras:</strong> {selected.cameras?.length ?? 0}</li>
                  <li><strong>Enlazado:</strong> {selected.linked ? "Si" : "No"}</li>
                </ul>
                <Button variant="secondary" onClick={() => setActiveSection("Cruces")}>Abrir ficha en Cruces</Button>
              </div>
            ) : (
              <div className="map-fullview-hud map-fullview-hud-empty">
                <span className="map-fullview-hud-tag">Sin selección</span>
                <p>Pulsa un marcador del mapa para verlo aqui o cambia a la seccion Cruces.</p>
                <Button onClick={guardAction("editGeometry", "activar modo anadir cruce", () => setAddMode((value) => !value))} variant={addMode ? "primary" : "secondary"}>
                  {addMode ? "Cancelar añadir" : "Modo añadir cruce"}
                </Button>
              </div>
            )}
          </div>
    ...
    if (activeSection === "Corredores") {
      return (
        <SectionPage title="Corredores" help={[
    ...
    <main>
      <div className="hydra-shell">
        ...
        <LoginModalR26
          ...
          pin={loginPin}
          error={loginError}
          onUserChange={(userId) => { setLoginUserId(userId); setLoginError(""); }}
